...
async function executeSELECTQuery(query) {
    ...
    if (hasAggregateWithoutGroupBy && fields.length == 1) {
        const selectedEntry = {};
        selectedEntry[fields[0]] = aggregatedOperations(fields[0], filteredData);
    ...
    return filteredData.map((row) => {
        ...
        fields.forEach((field) => {
            ...
            selectedEntry[field] = aggregatedOperations(field, filteredData);
            } else {
            selectedEntry[field] = row[field];
            }
    });
    return selectedEntry;
    });
